...
# ---------------------------------------------------------------------------------------------
# Function to get the string with the timestamp information for each file
def getTSstring(filename, filelist):
    ...
    for line in search_file:
        ...
        # If we got a match then get the string 
        if line.strip().find(filename) != -1:
            string = line
            
            # check if the string is flagged as bad
            if "bad" in string:
                string="bad"
                print("Error bad file: ",filename)
...
# The input list with the timestamps
input_ts_list="/lus/theta-fs0/projects/uboone/NuMI_Data/run1/beamoff/event_lists/prod_extnumi_swizzle_inclusive_v3_tmnt_stride2_offset0_goodruns_mcc9_FileNames_timestamps.txt"
...
for i, _file in enumerate(files):
    ...
    # Here we get the string with the timestamps, if bad is returned then skip the file
    ts_string = getTSstring(join_args_full, input_ts_list)
